<nav class="bg-white shadow-md sticky top-0 z-50">
            ...
            <div class="flex items-center">
                ...
                <div class="hidden md:block">
                    <div class="mr-10 flex items-baseline space-x-4 space-x-reverse">
                        <a href="{{ route('home') }}"
                            class="nav-link text-gray-700 hover:text-primary px-3 py-2 rounded-md text-sm font-medium transition duration-300">الرئيسية</a>
                        <a href="{{ route('projects.main') }}"
                            class="nav-link text-gray-700 hover:text-primary px-3 py-2 rounded-md text-sm font-medium transition duration-300">المشاريع</a>
                        <a href="{{ route('freelance.main') }}"
                            class="nav-link text-gray-700 hover:text-primary px-3 py-2 rounded-md text-sm font-medium transition duration-300">فري
                            لانسر</a>
                        <a href="{{ route('company.main') }}"
                            class="nav-link text-gray-700 hover:text-primary px-3 py-2 rounded-md text-sm font-medium transition duration-300">الشركات
                            والمكاتب واصحاب العمل</a>
                    </div>
                </div>
            </div>
                ...
                @auth
                    ...
                    @if (auth()->user()->role === 'freelance')
                        ...
                        <a href="{{ route('freelancer.dashboard') }}"
                            class="bg-primary hover:bg-secondary text-white px-4 py-2 rounded-md text-sm font-medium transition duration-300 transform hover:scale-105">
                            لوحة تحكم صاحب عمل حر
                        </a>
                    @else
                        <!-- إذا كان عميل -->
                        <a href="{{ route('freelancer.dashboard') }}"
                            class="nav-link text-gray-700 hover:text-primary px-3 py-2 rounded-md text-sm font-medium transition duration-300">
                            لوحة التحكم
                        </a>
                    @endif
                    ...
                    <form method="POST" action="{{ route('logout') }}"
                        class="flex items-center justify-center text-center {{ Route::is('home') ? 'mt-4' : '' }}">
                        @csrf
                        <button type="submit"
                            class="nav-link text-gray-700 hover:text-primary px-3 py-2 rounded-md text-sm font-medium transition duration-300">
                            تسجيل الخروج
    ...
    <div id="mobile-menu" class="md:hidden hidden bg-white shadow-lg absolute top-16 left-0 w-full z-40">
        <div class="px-2 pt-2 pb-3 space-y-1 sm:px-3">
            <a href="{{ route('home') }}"
                class="text-gray-700 hover:text-primary block px-3 py-2 rounded-md text-base font-medium transition duration-300">الرئيسية</a>
            <a href="{{ route('projects.main') }}"
                class="text-gray-700 hover:text-primary block px-3 py-2 rounded-md text-base font-medium transition duration-300">المشاريع</a>
            <a href="{{ route('freelance.main') }}"
                class="text-gray-700 hover:text-primary block px-3 py-2 rounded-md text-base font-medium transition duration-300">اصحاب
                العمل الحر</a>
            <div class="border-t border-gray-200 pt-4">
                @auth
                    @if (auth()->user()->role === 'freelance')
                        <a href="{{ route('freelancer.dashboard') }}"
                            class="bg-primary hover:bg-secondary text-white block px-3 py-2 rounded-md text-base font-medium mt-2 transition duration-300 text-center">
                            لوحة تحكم صاحب عمل حر
                        </a>
                    @else
                        <a href="#"
                            class="text-gray-700 hover:text-primary block px-3 py-2 rounded-md text-base font-medium transition duration-300">
                            لوحة التحكم
                        </a>
                    @endif
                    <form method="POST" action="{{ route('logout') }}"
                        class="text-gray-700 hover:text-primary block px-3 py-2 rounded-md text-base font-medium transition duration-300 w-full text-center">
                        @csrf
                ...
                @else
                    <a href="#" id="loginBtnMobile"
                        class="text-gray-700 hover:text-primary block px-3 py-2 rounded-md text-base font-medium transition duration-300">تسجيل
                        الدخول</a>
                    <button id="joinFreelancerBtnMobile"
                        class="bg-primary hover:bg-secondary text-white block px-3 py-2 rounded-md text-base font-medium mt-2 transition duration-300 w-full text-center">
                        انضم إلينا
                    </button>
                @endauth
            </div>
...
@guest
            ...
            <form action="{{ route('register') }}" method="POST" class="mt-4">
                ...
                <div class="flex flex-col gap-3">
                    ...
                    <div class="text-center text-gray-600 text-sm">
                        هل لديك حساب؟
                        <a href="#" id="switchToLogin" class="text-primary hover:text-secondary font-bold">
    ...
    <!-- Modal تسجيل الدخول صاحب عمل حر Login -->
            ...
            <form action="{{ route('storeRole') }}" method="POST" class="mt-4">
                ...
                <div class="mb-4">
                    @if (Route::is('freelance.main'))
                        <input type="hidden" name="role" id="role" value="freelance">
                    @elseif (Route::is('project.main'))
                        <input type="hidden" name="role" id="role" value="project">
                    @elseif (Route::is('home'))
                        <input type="hidden" name="role" id="role" value="freelance">
                    @endif
                    <label for="email2" class="block text-gray-700 text-sm font-bold mb-2 text-right">البريد الالكتروني او رقم الهاتف</label>
                ...
                <div class="mb-6">
                    <label for="password2" class="block text-gray-700 text-sm font-bold mb-2 text-right">كلمة
                        المرور</label>
                ...
                <div class="flex flex-col gap-3">
                    ...
                    <div class="text-center text-gray-600 text-sm">
                        ليس لديك حساب؟
                        <a href="#" id="switchToRegister" class="text-primary hover:text-secondary font-bold">
                            سجل الآن كمقدم خدمة
                        </a>
                    </div>
                </div>
    ...
    <!-- Modal تسجيل الدخول صاحب مشروع Login -->
    <div id="loginProjectModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden overflow-y-auto h-full w-full z-50">
        <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white animate-fade-in">
            <div class="flex justify-between items-center mb-4">
                <h3 class="text-lg font-bold text-gray-900">تسجيل الدخول صاحب مشروع</h3>
                <button id="closeLoginProjectModal" class="text-gray-400 hover:text-gray-500">
                    <i class="fas fa-times text-xl"></i>
                </button>
            </div>
            ...
            <form action="{{ route('storeRole') }}" method="POST" class="mt-4">
                ...
                <div class="flex flex-col gap-3">
                    ...
                    <div class="text-center text-gray-600 text-sm">
                        ليس لديك حساب؟
                        <a href="#" id="switchToRegisterProject" class="text-primary hover:text-secondary font-bold">
                            سجل الآن كصاحب مشروع
                        </a>
                    </div>
                </div>
            </form>
...
@endguest
    ...
    <!-- الـ Modal -->
<div id="modal" class="fixed inset-0 z-50 hidden flex items-center justify-center bg-black bg-opacity-50">
    <div class="bg-white rounded-lg shadow-xl w-full max-w-md mx-4">
        <div class="flex justify-between items-center p-6 border-b">
            <h3 class="text-lg font-bold text-gray-900">انضم كمقدم خدمة</h3>
